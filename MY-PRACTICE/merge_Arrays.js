...
const mergeArrays = () => {
  // Converting first array into obj
  const nameMappings = arr1.reduce((acc, current) => {
  ...
  for (let i = 0; i < arr2.length; i++) {
    const { name, ...rest } = arr2[i];
    if (!nameMappings.hasOwnProperty(name)) {
      nameMappings[name] = rest;
      continue;
    }

    // Obtain the current elem
    const currentElem = nameMappings[name];

    // Traverse keys & compare inside the obj
    for (let elem in rest) {
      currentElem[elem] = currentElem.hasOwnProperty(elem)
        ? currentElem[elem] + rest[elem]
        : rest[elem];
    }
  }
